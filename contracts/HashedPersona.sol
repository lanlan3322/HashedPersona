...
contract HashedPersona is ERC721URIStorage, ReentrancyGuard {
    ...
    function fetchMarketItems() public view returns (ListedToken[] memory) {
    ...
    function fetchItemsListed(address _issuer) public view returns (ListedToken[] memory) {
        uint totalItemCount = _tokenIds.current();
        uint itemCount = 0;
        uint currentIndex = 0;
    ...
    function fetchMyNFTs(address _owner) public view returns (ListedToken[] memory) {
        uint totalItemCount = _tokenIds.current();
        uint itemCount = 0;
        uint currentIndex = 0;
